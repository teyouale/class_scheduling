...
const Departement = ({ handleMessage, index }) => {
	...
	const [timeInteval, setTimeInteval] = useState(0);
	...
	const handleTimeInterval = (event) => {
				...
				<AccordionSummary
					...
					<div className={classes.column}>
						<Typography className={classes.heading}>
							{departementName != "" ? departementName : "Departement"}
						</Typography>
					</div>
					<div className={classes.column}>
									...
									{courseFields.map((courseField, index) => (
										<FormGroup className={classes.tableCenter} key={index}>
											...
											<TextField
												...
												value={courseField.creditHour}
												onChange={(event) => handleInputChange(index, event)}
												label='Credit Hour'
												style={{ margin: 8 }}
												placeholder='Enter Your Credit Hour'
												InputLabelProps={{ shrink: true }}
												variant='filled'
											/>
											<TextField
												id='filled-full-width'
												name='courseInstactor'
												value={courseField.courseInstactor}
												onChange={(event) => handleInputChange(index, event)}
												label='Instractor'
												style={{ margin: 8 }}
												placeholder='Enter Your Instractor'
												InputLabelProps={{ shrink: true }}
												variant='filled'
											/>
											{courseField.length !== 1 && (
												<IconButton
													onClick={() => handleCourseRemoveClick(index)}>
													<RemoveIcon />
												</IconButton>
											)}
									...
									{classRoomFields.map((classRoomField, index) => (
										<FormGroup className={classes.tableCenter} key={index}>
											...
											<TextField
												id='filled-full-width'
												name='roomCapacity'
												value={classRoomField.roomCapacity}
												label='Capacity'
												onChange={(event) => handleInputChange(index, event)}
												style={{ margin: 8 }}
											...
											{classRoomField.length !== 1 && (
												<IconButton
													onClick={() => handleRoomRemoveClick(index)}>
													<RemoveIcon />
												</IconButton>
											)}
									...
									<Grid container>
										...
										<Grid item xs={6}>
											<TextField
												...
												name='courseInstactor'
												// value={"courseField.courseInstactor"}
												onChange={(event) => handleTimeInterval(event)}
												label='Time Inteval'
												style={{ margin: 8 }}
