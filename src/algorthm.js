...
var duration = 60; // accept input
var numberOfClassPerDay = 7; // accept input
var startTime = "2:00"; // accept input
...
for (let i = 0; i < numberOfClassPerDay - 1; i++) {
	var temp = meetingTime[meetingTime.length - 1].split(" - ");
	startTime = temp[1];
	var time = startTime.split(":");
	var hour = parseInt(time[0]) + (parseInt(time[1]) + duration) / 60;
	if (hour > 9) {
					...
					for (var a = 0; a < dep.length; a++) {
						if (dep[a].schedule[i][j] != null)
							if (getIndex(dep[a].schedule[i][j], dep[num].courses) != -1)
								if (
									a != num &&
									dep[a].CourseTeachermapping[
										getIndex(dep[a].schedule[i][j], dep[num].courses)
									] == dep[num].CourseTeachermapping[x]
								) {
									conflict = true;
									break;
								}
				...
				while (true) {
					...
					for (var a = 0; a < dep.length; a++) {
						if (dep[a].assignedRooms[i][j] != null)
							if (
								a != num &&
								dep[a].assignedRooms[i][j] == dep[num].classRooms[x]
							) {
								conflict = true;
								break;
							}
					...
					if (
						!conflict &&
						dep[num].roomCapacity[x] >= dep[num].numberOfStudent
					) {
